...
class Codegen(CodegenBase, TypesMapMixin, ConstFoldMixin, RuntimeBuiltinsMixin, FilesMixin, SetsMixin, StringsMixin, IoWriteReadMixin, StmtsMixin, DeclsMixin, ExprsMixin):
    ...
    def __init__(self, verbose: bool = False, source_file: Optional[str] = None, force_flags: Optional[Dict[str, bool]] = None, features: Optional[Dict[str, bool]] = None, device_triple: str = "x86_64-pc-linux-gnu"):
        ...
        super().__init__(verbose=verbose, source_file=source_file, force_flags=force_flags, features=features, device_triple=device_triple)
...
def compile_to_llvm(
    ...
    codegen = Codegen(verbose=verbose, source_file=source_file, force_flags=merged or None, features=features, device_triple=device_triple)
