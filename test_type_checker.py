...
def run_type_checker(source: str) -> tuple:
    ...
    try:
        result = subprocess.run(
            ['python3', '/home/ubuntu/pascal-1981/compile_to_llvm.py',
             pas_file, '/dev/null'],
            capture_output=True,
            text=True
        )
...
def main():
    ...
    tests = [
        ...
        ("Scope isolation - procedure",
         "PROGRAM P; PROCEDURE P1; VAR x: INTEGER; BEGIN END; BEGIN WRITELN(x) END.",
         False, "Undefined variable"),
        
        ("Const declaration (type checking only)",
         "PROGRAM P; CONST x = 42; BEGIN END.",
         True, None),
        
        ("Shadowing in nested scope",
         "PROGRAM P; VAR x: INTEGER; PROCEDURE P1; VAR x: INTEGER; BEGIN x := 1 END; BEGIN x := 2 END.",
         True, None),
    ]
    ...
    tests = [
        ...
        ("REAL to INTEGER assignment (error)",
         "PROGRAM P; VAR x: INTEGER; BEGIN x := 3.14 END.",
         False, "Cannot assign REAL"),
        
        ("INTEGER to BOOLEAN assignment (error)",
         "PROGRAM P; VAR b: BOOLEAN; BEGIN b := 1 END.",
         False, "Cannot assign INTEGER"),
        
        ("BOOLEAN type checking in expression",
         "PROGRAM P; VAR b: BOOLEAN; VAR x: INTEGER; BEGIN IF b THEN x := 1 END.",
         True, None),
        
        ("Multiple assignments",
         "PROGRAM P; VAR x: INTEGER; BEGIN x := 1; x := 2; x := 3 END.",
         True, None),
    ]
    for name, src, should_pass, err in tests:
        total += 1
    ...
    tests = [
        ("IF with BOOLEAN condition",
         "PROGRAM P; BEGIN IF TRUE THEN WRITELN(1) END.",
         True, None),
        
        ("IF with INTEGER condition (error)",
         "PROGRAM P; BEGIN IF 42 THEN WRITELN(1) END.",
        ...
        ("WHILE with INTEGER condition (error)",
        ...
        ("FOR loop with REAL variable (error)",
    ...
    tests = [
        ("Undefined procedure call (error)",
         "PROGRAM P; BEGIN UNDEFINED_PROC() END.",
         False, "Undefined"),
        
        ("Valid procedure call",
         "PROGRAM P; PROCEDURE P1; BEGIN END; BEGIN P1 END.",
         True, None),
        ...
        ("Undefined function (error)",
         "PROGRAM P; BEGIN WRITELN(UNDEFINED_FUNC()) END.",
         False, "Undefined"),
    ]
    for name, src, should_pass, err in tests:
        total += 1
    ...
    tests = [
        ("Mismatched return type (error)",
         "PROGRAM P; FUNCTION F: INTEGER; BEGIN F := 3.14 END; BEGIN END.",
         False, "Cannot assign REAL"),
    ]
    for name, src, should_pass, err in tests:
        total += 1
    ...
    tests = [
        ("Array with REAL index (error)",
         "PROGRAM P; VAR a: ARRAY[1..10] OF INTEGER; BEGIN a[1.5] := 42 END.",
         False, "Array index must be INTEGER"),
    ]
    for name, src, should_pass, err in tests:
        total += 1
        if test_case(name, src, should_pass, err):
    ...
    tests = [
        ("Integer arithmetic type (valid via type inference)",
         "PROGRAM P; VAR x: INTEGER; BEGIN x := 1 + 2 END.",
         True, None),
        
        ("Mixed type operations (error)",
    ...
    tests = [
        ...
        ("Const with undefined use (error)",
         "PROGRAM P; CONST z = 10; BEGIN WRITELN(y) END.",
         False, "Undefined"),
        
        ("Procedure with parameter type checking",
