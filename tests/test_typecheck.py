...
class TestReadWriteTypecheck(unittest.TestCase):
    ...
    def test_fcb_trap_errs_accepted_on_file_variables(self):
        """F.TRAP / F.ERRS on file variables now typecheck (trapped I/O is
        implemented): TRAP is BOOLEAN, ERRS is INTEGER. Direct FCBFQQ record
        variables keep working.

        HISTORY: this test originally asserted *rejection* with a 'not yet
        supported' diagnostic, from the era when these fields had no codegen
        lowering. Flipped (not deleted) when trapped I/O landed."""
        trap = typecheck_source("PROGRAM P; VAR f: TEXT; b: BOOLEAN; BEGIN b := f.TRAP END.")
        self.assertTrue(trap.success, msg=" ".join(str(e) for e in trap.errors))
        errs = typecheck_source("PROGRAM P; VAR f: TEXT; BEGIN f.ERRS := 0 END.")
        self.assertTrue(errs.success, msg=" ".join(str(e) for e in errs.errors))
        wrong = typecheck_source("PROGRAM P; VAR f: TEXT; BEGIN f.TRAP := 3 END.")
        self.assertFalse(wrong.success)  # TRAP is BOOLEAN, not INTEGER
...
class TestCallValidation(unittest.TestCase):
    ...
    def test_string_edit_intrinsics_typecheck(self):
        ...
        result = typecheck_source(
            "PROGRAM P; VAR s: STRING(10); VAR t: STRING(10); VAR l: LSTRING(10); VAR n: INTEGER; BEGIN INSERT(s, t, 1); DELETE(t, 1, 1); WRITELN(POSITN(t, s)); WRITELN(SCANEQ(1, 'a', s, 1)); WRITELN(SCANNE(1, 'a', s, 1)); WRITELN(ENCODE(l, n)); WRITELN(DECODE(s, n)) END."
        )
        self.assertTrue(result.success, msg=" ".join(str(e) for e in result.errors))
...
class TestWideReals(unittest.TestCase):
    """REAL32 (32-bit float) and REAL64 (64-bit synonym for REAL) extension types.

    Gated behind -f wide-reals in host code (always available in DEVICE code).
    """

    from pascal1981.features import resolve_features
    _WR = resolve_features('vintage', ['wide-reals'])

    def test_real32_real64_rejected_without_flag(self):
        result = typecheck_source("PROGRAM P; VAR a: REAL32; BEGIN END.")
        self.assertFalse(result.success)
